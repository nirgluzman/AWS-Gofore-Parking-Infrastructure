...
resource "aws_cognito_user_pool" "main" {
  ...
  password_policy {
    minimum_length    = 6 # minimum length of the password
    require_lowercase = false # whether you've required users to use at least one lowercase letter in their password
...
resource "aws_cognito_user_pool_client" "main" {
  ...
  name                  = var.user_pool_client_name # name of the application client (friendly identifier)
  ...
  callback_urls         = var.user_pool_client_callback_urls # URLs in application that Cognito redirects users to after successful authentication
...
resource "aws_cognito_user_pool_domain" "main" {
  domain = var.user_pool_domain_prefix # the domain prefix
